...
interface Props { ticketId: number; canManage: boolean; refreshKey?: number; draftText?: string; onChanged?(): void }

export function TicketConversation({ ticketId, canManage, refreshKey = 0, draftText = '', onChanged }: Props) {
  ...
  const [reply, setReply] = useState(''); const [note, setNote] = useState(''); const [error, setError] = useState<string | null>(null); const [busy, setBusy] = useState(false)
  useEffect(() => {
  ...
  useEffect(() => { if (draftText) { setReply(draftText); setTab('conversation') } }, [draftText])
  ...
  async function sendReply() { if (!reply.trim()) return; setBusy(true); try { await addAgentMessage(ticketId, reply.trim()); setReply(''); setMessages(await fetchTicketMessages(ticketId)); onChanged?.() } catch (e) { setError((e as Error).message) } finally { setBusy(false) } }
  ...
  return <section className="ticket-conversation">
    ...
    {tab === 'conversation' ? <><div className="ticket-messages">{messages.length === 0 ? <p className="empty-copy">No conversation messages.</p> : messages.map(message => <article key={message.id} className={`ticket-message ${message.senderType.toLowerCase() === 'customer' ? 'customer' : 'agent'}`}><header><strong>{message.senderType}</strong><span>{new Date(message.createdAt).toLocaleString()}</span></header><p>{message.message}</p></article>)}</div>
      {canManage && <div className="agent-reply"><label>Agent Reply<textarea rows={4} maxLength={4000} value={reply} onChange={e => setReply(e.target.value)} placeholder="Write a reply to the customer…" /></label><button onClick={sendReply} disabled={busy || !reply.trim()}>{busy ? 'Sending…' : 'Send Reply'}</button><p className="panel-help">AI drafts are not sent until you click Send Reply.</p></div>}</> :
